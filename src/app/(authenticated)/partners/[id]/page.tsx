...
export default function PartnerDetailPage() {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[60vh]"><div className="w-6 h-6 border-2 border-gold border-t-transparent rounded-full animate-spin" /><span className="ml-2 text-sm text-white/50 font-inter">Loading...</span></div>;
    ...
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-white" style={{ fontFamily: BRAND.fonts.playfair }}>{fullName}</h1>
            {partner.company && <p className="text-sm text-white/50 font-inter">{partner.company}{partner.role ? ` - ${partner.role}` : ''}</p>}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card className="!p-4 text-center"><Users size={16} className="text-gold mx-auto mb-1" /><p className="text-2xl font-bold text-white" style={{ fontFamily: BRAND.fonts.dmSerif }}>{partner.total_leads_sent}</p><p className="text-[10px] text-white/40 font-inter">Leads Sent</p></Card>
        <Card className="!p-4 text-center"><TrendingUp size={16} className="text-green-500 mx-auto mb-1" /><p className="text-2xl font-bold text-white" style={{ fontFamily: BRAND.fonts.dmSerif }}>{partner.total_closings}</p><p className="text-[10px] text-white/40 font-inter">Closings</p></Card>
        <Card className="!p-4 text-center"><DollarSign size={16} className="text-gold mx-auto mb-1" /><p className="text-2xl font-bold text-gold" style={{ fontFamily: BRAND.fonts.dmSerif }}>${(partner.total_revenue_generated || 0).toLocaleString()}</p><p className="text-[10px] text-white/40 font-inter">Revenue</p></Card>
        <Card className="!p-4 text-center"><DollarSign size={16} className="text-blue-500 mx-auto mb-1" /><p className="text-2xl font-bold text-white" style={{ fontFamily: BRAND.fonts.dmSerif }}>{partner.total_revenue_generated > 0 && totalReferralFees > 0 ? `${Math.round(partner.total_revenue_generated / totalReferralFees)}x` : 'N/A'}</p><p className="text-[10px] text-white/40 font-inter">ROI (Rev/Fees)</p></Card>
      </div>
      ...
      {totalReferralFees > 0 && (
        <Card className="!p-4 mb-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-montserrat font-medium text-white/70">Total Referral Fees Paid</span>
            <span className="text-lg font-bold text-white" style={{ fontFamily: BRAND.fonts.dmSerif }}>${totalReferralFees.toLocaleString()}</span>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card className="!p-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {partner.phone && <div className="flex items-center gap-3"><Phone size={14} className="text-gold" /><div><p className="text-xs text-white/40 font-inter">Phone</p><p className="text-sm font-inter text-white">{partner.phone}</p></div></div>}
              {partner.email && <div className="flex items-center gap-3"><Mail size={14} className="text-gold" /><div><p className="text-xs text-white/40 font-inter">Email</p><p className="text-sm font-inter text-white">{partner.email}</p></div></div>}
              {partner.language_preference && <div className="flex items-center gap-3"><Globe size={14} className="text-gold" /><div><p className="text-xs text-white/40 font-inter">Language</p><p className="text-sm font-inter text-white capitalize">{partner.language_preference}</p></div></div>}
              {partner.referral_fee_structure && <div className="flex items-center gap-3"><DollarSign size={14} className="text-gold" /><div><p className="text-xs text-white/40 font-inter">Fee Structure</p><p className="text-sm font-inter text-white">{partner.referral_fee_structure}</p></div></div>}
            </div>
            {partner.notes && <div className="mt-4 pt-4 border-t border-gold/10"><p className="text-xs text-white/40 font-inter mb-1">Notes</p><p className="text-sm font-inter text-white/70 whitespace-pre-wrap">{partner.notes}</p></div>}
          </Card>

          {/* Referred Contacts */}
          <Card className="!p-5">
            <h3 className="text-sm font-montserrat font-semibold text-white/70 mb-3">Referred Contacts ({contacts.length})</h3>
            {contacts.length > 0 ? (
              <div className="space-y-2">
                {contacts.map(c => (
                  <button key={c.id} onClick={() => router.push(`/contacts/${c.id}`)} className="w-full flex items-center justify-between p-3 rounded-lg bg-white/5 hover:bg-gold/5 transition-colors text-left">
                    <div>
                      <p className="text-sm font-montserrat font-medium text-white">{getDisplayName(c)}</p>
                      <p className="text-xs text-white/40 font-inter capitalize">{c.track_type} - {c.pipeline_stage.replace(/_/g, ' ')}</p>
                    </div>
                    <ChevronRight size={14} className="text-white/20" />
                  </button>
                ))}
              </div>
            ) : <p className="text-sm text-white/40 font-inter">No contacts referred yet.</p>}
          </Card>

          {/* Transactions from referrals */}
          {transactions.length > 0 && (
            <Card className="!p-5">
              <h3 className="text-sm font-montserrat font-semibold text-white/70 mb-3">Deals ({transactions.length})</h3>
              <div className="space-y-2">
                {transactions.map(tx => (
                  <button key={tx.id} onClick={() => router.push(`/transactions/${tx.id}`)} className="w-full flex items-center justify-between p-3 rounded-lg bg-white/5 hover:bg-gold/5 transition-colors text-left">
                    <div>
                      <p className="text-sm font-montserrat font-medium text-white">{tx.property_address}</p>
                      <p className="text-xs text-white/40 font-inter">{tx.contract_price ? `$${tx.contract_price.toLocaleString()}` : 'Price TBD'}</p>
                    </div>
        ...
        <div className="space-y-4">
          <Card className="!p-5">
            ...
            <div className="space-y-2">
              <div><p className="text-xs text-white/40 font-inter">Added</p><p className="text-sm font-inter text-white">{new Date(partner.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p></div>
            </div>
          </Card>
        </div>
      </div>
